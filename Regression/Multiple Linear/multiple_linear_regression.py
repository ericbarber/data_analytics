...
"""Carry on removing variables with p_values greater than significance level 0.05"""

"""# Assign variable from data processing to X values with p_values label

x_pValues = X

# p_value opperation for linear regression
import statsmodels.formula.api as sm
def backwardElimination(x_pValues, sl):
    numVars = len(x_pValues[0])
    for i in range(0, numVars):
        regressor_OLS = sm.OLS(y, x_pValues).fit()
        maxVar = max(regressor_OLS.pvalues).astype(float)
        if maxVar > sl:
            for j in range(0, numVars - i):
                if (regressor_OLS.pvalues[j].astype(float) == maxVar):
                    x_pValues = np.delete(x_pValues, j, 1)
    regressor_OLS.summary()
    return x_pValues
 
SL = 0.05
x_pValues_opt = x_pValues[:, [0, 1, 2, 3, 4, 5]]
x_pValues_Modeled = backwardElimination(x_pValues_opt, SL)"""

"""# Assign variable from data processing to X values with p_values label
x_pValues_adjRsquar = X

# Linear Regression with p_value and adjusted R squar correction. 
import statsmodels.formula.api as sm
def backwardElimination(x_pValues_adjRsquar, SL):
    numVars = len(x_pValues_adjRsquar[0])
    temp = np.zeros((50,6)).astype(int)
    for i in range(0, numVars):
        regressor_OLS = sm.OLS(y, x_pValues_adjRsquar).fit()
        maxVar = max(regressor_OLS.pvalues).astype(float)
        adjR_before = regressor_OLS.rsquared_adj.astype(float)
        if maxVar > SL:
            for j in range(0, numVars - i):
                if (regressor_OLS.pvalues[j].astype(float) == maxVar):
                    temp[:,j] = x_pValues_adjRsquar[:, j]
                    x_pValues_adjRsquar = np.delete(x_pValues_adjRsquar, j, 1)
                    tmp_regressor = sm.OLS(y, x_pValues_adjRsquar).fit()
                    adjR_after = tmp_regressor.rsquared_adj.astype(float)
                    if (adjR_before >= adjR_after):
                        x_pValues_adjRsquar_rollback = np.hstack((x_pValues_adjRsquar, temp[:,[0,j]]))
                        x_pValues_adjRsquar_rollback = np.delete(x_pValues_adjRsquar_rollback, j, 1)
                        print (regressor_OLS.summary())
                        return x_pValues_adjRsquar_rollback
                    else:
                        continue
    regressor_OLS.summary()
    return x
